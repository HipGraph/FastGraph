        ...
        while(nIntermediate > 1){
            ...
            while(idxs < nIntermediate){
                if(idxf < idxs){
                    ...
                    temp2 = tree[idxs];
                    clock.Start();
                    pvector<uint32_t> nnzCPerCol = symbolicSpAddRegularDynamic<uint32_t,uint32_t,uint32_t,uint32_t>(tree[idxf], tree[idxs]);
                    tree[j] = new CSC<uint32_t, uint32_t, uint32_t>(SpAddRegularDynamic<uint32_t,uint32_t,uint32_t,uint32_t>(tree[idxf], tree[idxs], nnzCPerCol));
                    clock.Stop();
