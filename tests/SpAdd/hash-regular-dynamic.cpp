...
int main(int argc, char* argv[]){
    ...
    for(int i = 0; i < threads.size(); i++){
        ...
        for(int it = 0; it < iterations; it++){
            clock.Start();
            nnzCPerCol = symbolicSpMultiAddHashDynamic<uint32_t, uint32_t, uint32_t, uint32_t, uint32_t>(vec);
            clock.Stop();
            hash_time += clock.Seconds();
            clock.Start();
            SpAdd_out = SpMultiAddHashDynamic<uint32_t,uint32_t, uint32_t,uint32_t> (vec, nnzCPerCol, true);
            clock.Stop();
            hash_time += clock.Seconds();
        }
        if(type == 0){
